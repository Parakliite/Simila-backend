...
const getTopRatedByMatches = `-- name: GetTopRatedByMatches :many
SELECT
  m.id            AS media_id,
  m.title         AS title,
...
type GetTopRatedByMatchesParams struct {
	TargetUserID uuid.UUID
	Limit        int32
}

type GetTopRatedByMatchesRow struct {
	MediaID     uuid.UUID
	Title       string
	PosterPath  string
	MediaType   string
	...
	MatchName   string
	MatchAvatar sql.NullString
	MatchScore  float64
	RatingValue int32
	RatedAt     time.Time
}

func (q *Queries) GetTopRatedByMatches(ctx context.Context, arg GetTopRatedByMatchesParams) ([]GetTopRatedByMatchesRow, error) {
	rows, err := q.db.QueryContext(ctx, getTopRatedByMatches, arg.TargetUserID, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetTopRatedByMatchesRow
	for rows.Next() {
		var i GetTopRatedByMatchesRow
		if err := rows.Scan(
			&i.MediaID,
			&i.Title,
			&i.PosterPath,
